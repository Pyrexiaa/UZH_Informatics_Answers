...
wa_nrs = ["0781111119", "0792653913", "0797763139", "0792793193", "0781139022", "0764320165", '0792663130']
...
def get_possible_nrs(n):
    ...
    for i in range(len(n)-2):
        temp = n[2:] # start after 07
        for digit in range(10):
            if i == 0:
                possible_number = str(digit) + temp
            elif i == 6:
                possible_number = temp + str(digit)
            else:
                possible_number = temp[:i] + str(digit) + temp[i:]
            
            if len(possible_number) == 8:
                possible_number = "07" + possible_number
                possible_nrs_for_juliet.append(possible_number)
    
    result = list(set(possible_nrs_for_juliet) & set(wa_nrs))
    return result
